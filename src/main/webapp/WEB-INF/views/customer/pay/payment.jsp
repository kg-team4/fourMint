...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/user/css/payment.css" />
		...
		<form name="cartForm" id="cartForm" action="order.do" method="post"
			...
			<div id="Contents" style="min-height: 150%;">
				...
				<div class="title_box"
					...
					<h1>
						장바구니<span class="tx_num"
							style="margin-left: 10px; background: #949997; font-size: 20px;">${fn:length(cart) }</span>
					</h1>
					<ul class="step_list">
						...
						<li class="last">&emsp; <span class="step_num tx_num">3</span>
							주문완료
						</li>
					</ul>
				</div>
				<!--// title_box -->
				<!-- 민트마켓 배송상품 -->
				<h2 class="sub-title2">민트스토어 결제상품</h2>
				<table class="tbl_prd_list tableFix">
					...
					<thead>
						<tr>
							<th scope="col"><input type="checkbox" checked
								id="inp_allRe1" name="" value="" title="민트마켓 배송상품 전체 선택" /> <script>
									$("#inp_allRe1")
											.click(
													function() {
														var chk = $(
					...
					<tbody>
						...
						<c:forEach var="cart" items="${cart }" varStatus="status">
									...
									<div class="tbl_cont_area">
										<div class="tbl_cell w40" style="width: 100px;">
											<input type="checkbox" checked name="checkBox"
												class="chkSmall" data-cartNum="${cart.cart_id }" />
											<script>
												$(".chkSmall")
														.click(
																function() {
										...
										<div class="tbl_cell w200" style="width: 100px;">
											...
											<div class="prd_cnt" style="display: flex;">
												<input disabled class="amount${status.index }" name="amount"
													...
													value="${cart.amount }"> <input type="button"
													class="plus" style="padding: 0; width: 13px; height: 13px"
													value="+" /> <input type="button" class="minus"
													style="padding: 0; width: 13px; height: 13px" value="-" />
										...
										<div class="tbl_cell w120  ">
											<p class="prd_delivery s">
												<strong class="deliStrongText${status.index }"> <c:choose>
														<c:when
															test="${(cart.product_price * cart.amount) > 50000 }">무료배송</c:when>
										...
										<div class="tbl_cell w150">
											<div class="btn_group">
												<input type="hidden" value="${cart.cart_id }" />
												<button type="button" class="btnSmall wGreen buyBtn"
													name="btnBuy">
													<span>바로구매</span>
												</button>
											</div>
							...
							<c:set var="sum"
								value="${sum + (cart.product_price * cart.amount)}" />
						</c:forEach>
					</tbody>
				</table>
				<!-- 민트마켓 배송상품 결제금액 -->
				<div class="basket_price_info">
					<div class="btn_area">
						<button type="button" class="btnSmall wGray type2"
							id="partDelBtn1" name="partDelBtn" style="margin-left:12px;">
							<span>선택상품 삭제</span>
						</button>
						<script>
							$("#partDelBtn1")
					...
					<div class="sum_price" id="reloadTest" style="margin-right:20px; margin-bottom:10px; width:800px; text-align:right; font-size:19px">
						총 판매가 <span class="tx_num" id="allPrice" style="font-size:19px">${sum}</span>원 <span
							class="tx_sign plus"> + </span> 배송비 <span class="tx_num"
							id="allDelivery" style="font-size:19px"> <c:choose>
						...
						</span> <span class="tx_sign equal" style="font-size:19px">=</span> <span class="tx_total_price" style="font-size:19px">총
							결제금액 <span class="tx_price"><span id="reallyPrice"
								class="tx_num" style="font-size:23px"> <c:choose>
										<c:when test="${sum >= 50000 }">
						<fmt:formatNumber type="number" maxFractionDigits="3" value="${sum}"/>
						</c:when>
										<c:otherwise>
										
						<fmt:formatNumber type="number" maxFractionDigits="3" value="${sum + 2500 }"/>
						
						</c:otherwise>
									</c:choose></span>원</span>
						</span>
					</div>
				</div>
...
<div id="footer_wrapper" style="position: relative;">
	<footer>
		...
		<div class="footer-center">
			<p class="link">
				<a href="useAgreement.do">이용약관</a>&nbsp; | &nbsp; <a
					href="privacyPolicy.do">개인정보처리방침</a>&nbsp; | &nbsp; <a
					href="locationPolicy.do">위치기반서비스</a>
			</p>
			<p>
				서울 종로구 돈화문로 26 단성사 4층 <br> 사업자 등록번호 : 010-0000-4919 &nbsp;
				(주)민트마켓 대표 이승재 <br> <span class="copyright">COPYRIGHT
					Mint Market Inc. ALL RIGHTS RESERVED.</span>
			</p>
		</div>
		<div class="footer-left">
			<p>
				대표이사, 개인정보 책임자 : 이승재 <br> TEL : 02-0000-0000 <br> FAX :
				02-000-0001 <br> E-MAIL : asd4919@daum.net
			</p>
		</div>
	</footer>
</div>
